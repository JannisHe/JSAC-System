function [s_ECA_B] = ECA_BParallel(xofdm, Nofdm, Mofdm, Delta_f, Tsym, s_surv, batch_size)
% ECA_BParallel: Implements the parallel version of the ECA-Batches (ECA-B) algorithm for clutter suppression
%
% Inputs:
...
% - batch_size: Number of samples in each batch for parallel processing
%
% Output:
% - s_ECA_B: Output signal after clutter suppression using the parallel ECA-B algorithm
%
% Author: Jannis Held
...
s_ECA_batches = cell(num_batches, 1);

% Parallel for-loop to process each batch
parfor batch_idx = 1:num_batches
    ...
    alpha_batch = (X_batch' * X_batch + lambda * eye(size(X_batch, 2))) \ (X_batch' * s_surv_batch);
    s_ECA_batch = s_surv_batch - X_batch * alpha_batch;
    
    s_ECA_batches{batch_idx} = s_ECA_batch;
end

% Combine all the processed batches into the final output signal
for batch_idx = 1:num_batches
    batch_start = (batch_idx - 1) * batch_size + 1;
    batch_end = min(batch_idx * batch_size, N);
    s_ECA_B(batch_start:batch_end) = s_ECA_batches{batch_idx};
end
end
